...
until [[ $user =~ ^[a-zA-Z0-9_]+$ && ${CLIENT_EXISTS} == '0' ]]; do
        ...
        echo -e "\E[0;36m     Add XRAY Vless WS Account     \E[0m"
        echo -e "\033[0;34m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\033[0m"
...
clear
		    echo -e "\033[0;34m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\033[0m"
            echo -e "\E[0;36m      Add XRAY Vless Account       \E[0m"
            echo -e "\033[0;34m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\033[0m"
			echo ""
...
vlesslink1="vless://${uuid}@${sts}${domain}:443?type=ws&encryption=none&security=tls&host=${domain}&path=/vless-ws&allowInsecure=1&sni=${sni}#XRAY_VLESS_TLS_${user}"
vlesslink2="vless://${uuid}@${sts}${domain}:80?type=ws&encryption=none&security=none&host=${domain}&path=/vless-ws#XRAY_VLESS_NON_TLS_${user}"
...
echo -e "Path TLS          : /vless-ws" | tee -a /usr/local/etc/xray/configlogs/vlessws-$user.txt
echo -e "Path NTLS         : /vless-ws" | tee -a /usr/local/etc/xray/configlogs/vlessws-$user.txt
echo -e "═══════════════════════════════════════" | tee -a /usr/local/etc/xray/configlogs/vlessws-$user.txt
...
echo -e "═══════════════════════════════════════" | tee -a /usr/local/etc/xray/configlogs/vlessws-$user.txt
echo -e "YAML WS TLS       : http://${MYIP2}:81/$user-VLESSTLS.yaml" | tee -a /usr/local/etc/xray/configlogs/vlessws-$user.txt
echo -e "═══════════════════════════════════════" | tee -a /usr/local/etc/xray/configlogs/vlessws-$user.txt
echo -e "YAML WS None TLS  : http://${MYIP2}:81/$user-VLESSNTLS.yaml" | tee -a /usr/local/etc/xray/configlogs/vlessws-$user.txt
echo -e "═══════════════════════════════════════" | tee -a /usr/local/etc/xray/configlogs/vlessws-$user.txt
...
echo -e "═══════════════════════════════════════" | tee -a /usr/local/etc/xray/configlogs/vlessws-$user.txt
echo -e "" | tee -a /usr/local/etc/xray/configlogs/vlessws-$user.txt
echo -e "cutepookie" | tee -a /usr/local/etc/xray/configlogs/vlessws-$user.txt
echo -e "" | tee -a /usr/local/etc/xray/configlogs/vlessws-$user.txt