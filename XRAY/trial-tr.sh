...
trojanlink1="trojan://${uuid}@${domain}:443?type=ws&security=tls&host=${domain}&path=/trojan-ws&sni=${domain}#XRAY_TROJAN_TLS_${user}"
trojanlink2="trojan://${uuid}@${domain}:80?type=ws&security=none&host=${domain}&path=/trojan-ws#XRAY_TROJAN_NTLS_${user}"
...
echo -e "════════════════════════════════════════" | tee -a /usr/local/etc/xray/configlogs/trojanws-$user.txt
echo -e "" | tee -a /usr/local/etc/xray/configlogs/trojanws-$user.txt
echo -e "cutepookie" | tee -a /usr/local/etc/xray/configlogs/trojanws-$user.txt
echo -e "" | tee -a /usr/local/etc/xray/configlogs/trojanws-$user.txt
